...
use crate::{canvas::TerminalInfo, exit::exit, process::handle_process, span::{Node, NodeData, Span, SpanChild, SpanDirection}, tty::spawn_interactive_process, Process, StateContainer, Vector2};

pub async fn create_span(state_container: StateContainer) -> Result<Arc<Mutex<Process>>, Box<dyn std::error::Error>> {
            ...
            Some(root) => {
                match &root.data {
                    NodeData::Void => {
                        ...
                        let mut span = Span::new(SpanDirection::Horizontal);
                    ...
                    NodeData::Span(span) => {
                        ...
                        match &mut parent.data {
                            NodeData::Void => {
                                return Err(format!("Parent: {:?} is void",&parent).into());
                            },
                            NodeData::Span(span) => {
                                span.children.push(SpanChild::new(Node::new(new_id, NodeData::Void)).with_size(1.0));
