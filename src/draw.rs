                ...
                let size = if is_last {
                    match direction {
                        SpanDirection::Horizontal => Vector2::new(
                            (parent_dimensions.size.x as f64 * ratio).ceil() as isize,
                            parent_dimensions.size.y,
                        ),
                        SpanDirection::Vertical => Vector2::new(
                            parent_dimensions.size.x,
                            (parent_dimensions.size.y as f64 * ratio).ceil() as isize,
                        ),
                    }
                } else {
                    match direction {
                        SpanDirection::Horizontal => Vector2::new(
                            (parent_dimensions.size.x as f64 * ratio).round() as isize,
                            parent_dimensions.size.y,
                        ),
                        SpanDirection::Vertical => Vector2::new(
                            parent_dimensions.size.x,
